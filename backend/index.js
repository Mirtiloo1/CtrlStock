...
app.get("/api/movements", async (req, res) => {
  try {
    const r = await pool.query(`SELECT m.id, m.timestamp, m.tipo, p.nome, p.uid_etiqueta FROM movimentacoes m LEFT JOIN produtos p ON m.produto_id = p.id ORDER BY m.timestamp DESC LIMIT 100`);
    res.json({ success: true, data: r.rows });
  } catch { res.status(500).json({ success: false }); }
});
