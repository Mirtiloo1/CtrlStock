...
app.post("/api/movements", async (req, res) => {
  ...
  try {
    ...
    // 2. SE NÃO EXISTE: Bufferiza e Retorna 404 (ESP32: Bip Longo + Vermelho)
    if (!prod.rows.length) {
...
app.delete("/api/products/:id", async (req, res) => {
  const { id } = req.params;
  try {
    // Registra exclusão antes de desativar
...
// Histórico com JOIN para pegar nomes mesmo de produtos excluídos
...
app.listen(port, () => console.log(`Rodando na porta ${port}`));const express = require("express");
const { Pool } = require("pg");
const cors = require("cors");
const bcrypt = require("bcryptjs");
const jwt = require("jsonwebtoken");
